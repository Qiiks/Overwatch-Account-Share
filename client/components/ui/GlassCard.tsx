...
const GlassCard = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded border bg-cyan-500/3 backdrop-blur-md border-cyan-400/20 shadow-lg shadow-black/50 text-card-foreground relative overflow-hidden",
        "before:absolute before:inset-0 before:bg-gradient-to-br before:from-cyan-400/5 before:to-transparent before:pointer-events-none",
        "after:absolute after:inset-0 after:bg-[linear-gradient(45deg,transparent_48%,rgba(0,255,255,0.1)_49%,rgba(0,255,255,0.1)_51%,transparent_52%)] after:bg-[length:20px_20px] after:pointer-events-none after:opacity-20",
        className,
      )}
      {...props}
      style={{
        boxShadow:
          "0 8px 32px rgba(0, 0, 0, 0.5), inset 0 1px 0 rgba(0, 255, 255, 0.1), 0 0 20px rgba(0, 255, 255, 0.1)",
        ...props.style,
      }}
    />
  ),
