...
const FETCH_INTERVAL_MS = parseInt(
  process.env.OTP_FETCH_INTERVAL_MS || "10000",
  10,
);
...
class OTPService {
  ...
  async getOAuth2Client(googleAccountId) {
    ...
    if (!account || !account.refresh_token) {
      throw new Error(
        `Google account or refresh token not found for ID: ${googleAccountId}`,
      );
    }
    ...
    const oauth2Client = new google.auth.OAuth2(
      ...
      process.env.GOOGLE_REDIRECT_URI ||
        `${process.env.API_URL || "http://localhost:5001"}/api/google-auth/otp/callback`,
    );
  ...
  extractRecipientEmail(headers) {
    try {
      ...
      if (toHeader && toHeader.value) {
        ...
        const match =
          toHeader.value.match(/<([^>]+)>/) ||
          toHeader.value.match(/([^\s<>,]+@[^\s<>,]+)/);
        if (match && match[1]) {
      ...
      const xOriginalToHeader = headers.find(
  ...
  async fetchOTPsForAllAccounts(io) {
    if (this.isFetching) {
      logger.debug(
        "[OTP Service] Fetch cycle already in progress, skipping new run",
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        `[OTP Service] Found ${googleAccounts.length} active Google accounts`,
      );
      ...
      for (const googleAccount of googleAccounts) {
        try {
          ...
          logger.error(
            `[OTP Service] Error fetching from Google account ${googleAccount.email}:`,
            error.message,
          );
          ...
          if (error.response && [400, 401].includes(error.response.status)) {
            logger.warn(
              `[OTP Service] Deactivating Google account ${googleAccount.id} due to auth error`,
            );
  ...
  async fetchOTPsFromGoogleAccount(googleAccount, io) {
    ...
    if (messages.length === 0) {
      if (process.env.NODE_ENV !== "production") {
        logger.debug(
          `[OTP Service] No recent unread verification emails found for ${googleAccount.email}`,
        );
      }
      return;
    }
    ...
    logger.info(
      `[OTP Service] Processing ${messages.length} verification emails from ${googleAccount.email}`,
    );
    ...
    for (const message of messages) {
      try {
        ...
        const recipientEmail = this.extractRecipientEmail(
        ...
        if (!recipientEmail) {
          if (process.env.NODE_ENV !== "production") {
            logger.debug(
              `[OTP Service] Could not extract recipient email from message ${message.id}`,
            );
          }
          continue;
        }
        ...
        if (process.env.NODE_ENV !== "production") {
          logger.debug(
            `[OTP Service] Message ${message.id} sent to: ${recipientEmail}`,
          );
        }

        // Find the exact Overwatch account by matching the exact email
        const overwatchAccount =
          await OverwatchAccount.findByEmail(recipientEmail);

        if (!overwatchAccount) {
          if (process.env.NODE_ENV !== "production") {
            logger.debug(
              `[OTP Service] No Overwatch account found for email: ${recipientEmail}`,
            );
          }
          continue;
        }
        ...
        if (!htmlBody) {
          if (process.env.NODE_ENV !== "production") {
            logger.debug(
              `[OTP Service] No HTML body found in message ${message.id}`,
            );
          }
          continue;
        }
        ...
        logger.info(
          `[OTP Service] Successfully updated OTP for account (${recipientEmail})`,
        );
        ...
        await gmail.users.messages
          ...
          .catch((err) => {
            ...
            if (process.env.NODE_ENV !== "production") {
              logger.debug(
                `[OTP Service] Could not mark message ${message.id} as read:`,
                err.message,
              );
            }
          });
        ...
        logger.error(
          `[OTP Service] Error processing message ${message.id}:`,
          error.message,
        );
      }
    }
  ...
  async updateAccountOTP(account, otp, io) {
    ...
    const updatedAccount = await OverwatchAccount.updateOverwatchAccount(
      account.id,
      {
        otp: otp,
  ...
  emitOTP(io, account, otp) {
    // Decrypt accountTag for client-side matching
    let decryptedAccountTag = account.accounttag;
    try {
      decryptedAccountTag = decrypt(account.accounttag);
    } catch (error) {
      logger.warn("[OTP Service] Failed to decrypt accountTag, using as-is", {
        error: error.message,
        accountId: account.id,
      });
      // If decryption fails, use the original value (might be plaintext)
    ...
    logger.info(
      `[OTP Service] Emitted OTP update for account ${decryptedAccountTag}`,
      {
