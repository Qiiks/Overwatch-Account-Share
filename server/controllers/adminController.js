...
const getStats = async (req, res) => {
  ...
  try {
    ...
    if (cachedStats) {
      ...
      queryDuration =
        Number(process.hrtime.bigint() - statsQueryStart) / 1000000;
    ...
    if (isNaN(totalUsers) || isNaN(totalAccounts)) {
      logger.error("Invalid count values:", {
        totalUsers: totalUsersRaw,
        totalAccounts: totalAccountsRaw,
      });
...
const updateUserStatus = async (req, res) => {
  try {
    ...
    if (
      (user.id || user._id).toString() === req.user.id &&
    ...
    const formattedUser = {
      ...
      status: (
        user.isapproved !== undefined ? user.isapproved : user.isApproved
      )
        ? "active"
        : "suspended",
      joinDate:
    ...
    const oldStatus = (
      ...
      ? "active"
      : "suspended";
    adminAuditLogger.logUserStatusChanged(
      req.user.id,
      req.user.username,
      user.id || user._id,
      user.username,
...
const updateUserRole = async (req, res) => {
  try {
    ...
    // Audit log the role change (important security event)
    const oldRole = user.isadmin || user.isAdmin ? "admin" : "user";
    adminAuditLogger.logUserRoleChanged(
      req.user.id,
      req.user.username,
      user.id || user._id,
      user.username,
...
const deleteUser = async (req, res) => {
  try {
    ...
    // Audit log the user deletion (critical security event)
    adminAuditLogger.logUserDeleted(
      req.user.id,
      req.user.username,
...
const createUser = async (req, res) => {
  try {
    ...
    adminAuditLogger.logUserCreated(
      req.user.id,
      req.user.username,
      user.id,
      user.username,
